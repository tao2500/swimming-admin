<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('电话')" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.telephone }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('项目')" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.item }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('会员')" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.member }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('班级制')" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.classtype }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('到期日期')" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ timeTrans(row.overtime) | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
...
<!--    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />-->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="$t('电话号')" prop="telephone">
        ...
        <el-form-item :label="$t('项目')" prop="item">
          <el-select v-model="temp.item" class="filter-item" placeholder="Please select">
            <el-option v-for="item in itemOptions" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('会员')" prop="member">
          <el-select v-model="temp.member" class="filter-item" placeholder="Please select">
            <el-option v-for="item in memberOptions" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('班级制')" prop="classtype">
...
<script>
import { fetchList, fetchPv, createArticle, deleteArticle, updateArticle } from '@/api/student'
...
export default {
  ...
  data() {
    return {
      ...
      itemOptions: ['自由泳', '蝶泳', '蛙泳', '仰泳', '花样游泳'],
      classtypeOptions: ['私教','小班制'],
      showReviewer: false,
      temp: {
        id: undefined,
        password: '',
      ...
      rules: {
        ...
        telephone: [{ type: 'string', required: true, message: 'telephone is required', trigger: 'blur' }],
        // member: [{ type: 'string', required: true, message: 'member is required', trigger: 'blur' }]
      },
